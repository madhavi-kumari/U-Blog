...
public class Application {
    ...
    private void createPost() {
        ...
        Post post = new Post(1, loggedInEmailId, tag, title, description, LocalDateTime.now());
        ...
        Thread thread2 = new Thread() {
            public void run() {
                try {
                    ...
                    LogWriter.writeLog(
                            DateTimeFormatter.format(post.getTimestamp()) + "\tNew post with title [ " +
                                    title + " ] created by [ " + loggedInEmailId  + " ]",
    ...
    private void searchPost() {
        ...
        try {
            ...
            if (posts.isEmpty()) {
                ...
                System.out.println("Posts for " + emailId);
                for (Post post: posts) {
    ...
    private void deletePost() {
        ...
        try {
            boolean isDeleted = postService.deletePost(postId, loggedInEmailId);
            if (isDeleted) {
    ...
    private void filterPost() {
        ...
        try {
            System.out.println("All tags available: " + postService.getAllTags());
            System.out.print("Tag: ");
            String tag = scanner.nextLine();
            ...
            if (posts.isEmpty()) {
                ...
                System.out.println("Posts for " + tag);
                for (Post post: posts) {
                    System.out.println(post);
                    System.out.println("---------------------------");
                }
